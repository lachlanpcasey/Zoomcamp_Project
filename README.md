...
6. In DBT Cloud, configure your project to access the DBT folder within this github repo. This can be done by using git clone on this repository, making sure to make 'DBT' as the project subdirectory. (Please note, you will need to have a **paid** developer subscription to DBT cloud for this method to work). Please note that only the core_crypto_all_data model is **clustered** on the column called 'crypto', and **partitioned** on the column 'date'. It is clustered on the crypto column as this is what is used in my report as a parameter custom query, and so this table if often filtered on crypto. It is partitioned on the Date column so that particular dates can be referenced in the table easily. This optimisation was only needed on the core_crypto_all_data model, as this is the only table large enough to see noticeable efficiency improves.
